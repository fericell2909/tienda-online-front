        ...
        <div className="grid grid-cols-2 gap-6">
          <div>
            ...
            <ProductDetails
              ...
              isNew={data?.features?.stats?.isNew}
            />
          </div>
          <div>
            <span className="block text-gray-500 text-sm mb-2">
              {data?.features?.stats?.isNew ? 'Nuevo' : 'Usado'} |{' '}
              {data?.features?.stats?.sold} vendidos
            </span>
            <h1 className="text-xl lg:text-2xl font-semibold leading-7 lg:leading-6 text-gray-800 mb-4">
              {data.product_name}
            </h1>
            <div className="flex items-center gap-2 mb-4">
              <ProductRating rating={data?.features?.stats?.rating} />
              {data?.features?.stats?.sold > 300 && (
                <Badge text="Lo mas vendido" />
              )}
              {data?.features?.stats?.isNew && (
            ...
            <ProductInformation
              ...
              deliveryAvailable={data?.features?.stats?.hasDelivery}
            />
